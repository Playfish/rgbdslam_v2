                                      ...
                                      const sensor_msgs::CameraInfoConstPtr& cam_info_msg) 
{
  ...
  //cv::Mat visual_img =  bridge.imgMsgToCv(visual_img_msg);
    cv_bridge::CvImagePtr depth_float_img;
    try{ 
     depth_float_img = cv_bridge::toCvCopy(depth_img_msg, sensor_msgs::image_encodings::TYPE_32FC1);
    }
  ...
  if(visual_img.rows != depth_float_img->image.rows || 
     visual_img.cols != depth_float_img->image.cols){
    ROS_WARN("depth and visual image differ in size! Rescaling Depth Data");
    cv::resize(depth_float_img->image, depth_float_img->image, visual_img.size(), 0,0,cv::INTER_NEAREST);
    //return;
  }
  image_encoding_ = visual_img_msg->encoding;

  depthToCV8UC1(depth_float_img->image, depth_mono8_img_); //float can't be visualized or used as mask in float format TODO: reprogram keypoint detector to use float values with nan to mask
  ...
  noCloudCameraCallback(visual_img, depth_float_img->image, depth_mono8_img_, visual_img_msg->header, cam_info_msg);
}
